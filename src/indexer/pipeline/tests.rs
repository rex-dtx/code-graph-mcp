...
/// Three files export `save`; the caller imports exactly one of them and calls
/// it bare. All three tie on longest-common-path-prefix with the caller, so
/// `refine_ambiguous_targets` hands back the whole pool — the fan-out shape.
fn import_narrowing_fixture(src: &std::path::Path, caller_body: &str) {
    fs::create_dir_all(src).unwrap();
    for i in 1..=3 {
        fs::write(
            src.join(format!("def{i}.ts")),
            format!("export function save(x: number) {{ return x + {i}; }}\n"),
        )
        .unwrap();
    }
    fs::write(
        src.join("caller.ts"),
        format!("import {{ save }} from './def2';\n\nexport function run(x: number) {{\n{caller_body}\n}}\n"),
    )
    .unwrap();
}

/// BARE call edges only. The qualified-carve-out fixture also emits one
/// `{"q":"recv"}` edge per definer, and counting those made the original
/// assertion (`> 1 target`) true no matter what the narrowing did — both
/// carve-out mutations passed against it. Narrowing is gated on empty metadata
/// (prune's own eligibility rule), so the bare rows are the only ones that can
/// move.
fn bare_call_edge_targets(db: &Database) -> Vec<String> {
    let (_, edges) = graph_projection(db);
    let mut t: Vec<String> = edges
        .iter()
        .filter(|(sp, _, r, _, m)| {
            sp == "src/caller.ts"
                && r == REL_CALLS
                && m.as_deref().map(str::is_empty).unwrap_or(true)
        })
...
#[test]
fn test_import_narrowing_binds_to_the_imported_definer_only() {
    // `import_narrowed_targets` applies prune's verdict before the edges exist.
    // Its output must be exactly what prune would have left, so the observable
    // contract is the final edge set — the optimization is invisible in the
    // graph and only shows up in how much work was done to get there
    // (measured separately: 2.79s -> 0.18s on a 600-file, 60-definer corpus).
    let project_dir = TempDir::new().unwrap();
    let db_dir = TempDir::new().unwrap();
    let src = project_dir.path().join("src");
    import_narrowing_fixture(&src, "  return save(x);");

    let db = Database::open(&db_dir.path().join("index.db")).unwrap();
    run_full_index(&db, project_dir.path(), None, None).unwrap();

    assert_eq!(
        bare_call_edge_targets(&db),
        vec!["src/def2.ts:save".to_string()],
        "a bare call in a file that imports ONE same-named definer must bind to it alone"
    );
}

#[test]
fn test_import_narrowing_honours_prunes_qualified_call_carveout() {
    // The carve-out that makes this falsifiable. prune refuses to touch an edge
    // when the caller's source also contains a qualified `.save(` — the bare
    // call may legitimately bind elsewhere, so it keeps the whole fan-out
    // (biasing toward keeping edges). The narrowing must refuse for the same
    // reason; if it does not, it deletes edges prune would have KEPT, which is
    // a correctness regression wearing a performance badge.
    let project_dir = TempDir::new().unwrap();
    let db_dir = TempDir::new().unwrap();
    let src = project_dir.path().join("src");
    import_narrowing_fixture(
        &src,
        "  const obj = getObj();\n  obj.save(x);\n  return save(x);",
    );
    ...
    let db = Database::open(&db_dir.path().join("index.db")).unwrap();
    run_full_index(&db, project_dir.path(), None, None).unwrap();
    // Exact set, not a `> 1` count: the fan-out prune deliberately preserves is
    // ALL THREE definers. Anything less means the narrowing overrode a carve-out.
    assert_eq!(
        bare_call_edge_targets(&db),
        vec![
            "src/def1.ts:save".to_string(),
            "src/def2.ts:save".to_string(),
            "src/def3.ts:save".to_string(),
        ],
        "qualified `.save(` in the caller must disable narrowing — prune keeps these edges"
    );
}

#[test]
fn test_import_narrowing_carveouts_unit() {
    // Direct unit coverage for `import_narrowed_targets`, built the same way as
    // `test_prune_keeps_edge_when_caller_content_truncated`: a 4 KB source
    // fixture just to trip the truncation sentinel is far more machinery than
    // stating the row shape outright.
    //
    // These carve-outs need their OWN guard because the surrounding pipeline
    // hides narrowing mistakes: `bind_calls_to_imported_targets` re-adds the
    // import-bound edge and prune drops the contradicted one, so picking the
    // wrong subset self-heals (measured: a mutation that ignores the import set
    // entirely still yields a byte-identical 1680-edge graph). A carve-out
    // VIOLATION does not self-heal — it deletes an edge prune meant to keep and
    // bind will never restore.
    use super::resolve::{import_narrowed_targets, ImportNarrowCache};
    ...
    let caller_file = mk_file("src/caller.py");
    let d1 = mk_fn(mk_file("src/d1.py"), "save", "def save(): pass");
    let d2 = mk_fn(mk_file("src/d2.py"), "save", "def save(): pass");
    let d3 = mk_fn(mk_file("src/d3.py"), "save", "def save(): pass");
    let candidates = vec![d1, d2, d3];

    // The caller's file imports d2's `save` — the fact narrowing acts on.
    let importer = mk_fn(caller_file, "<module>", "import save from d2");
    insert_edge(conn, importer, d2, REL_IMPORTS, None).unwrap();

    let narrow = |code: &str, cache: &mut ImportNarrowCache| {
        let caller = mk_fn(caller_file, "run", code);
        import_narrowed_targets(&db, "src/caller.py", "save", &[caller], &candidates, cache)
            .unwrap()
    };

    // Happy path: bare caller, import present -> narrow to the imported definer.
    assert_eq!(
        narrow("def run(): save()", &mut ImportNarrowCache::default()),
        Some(vec![d2]),
        "a bare caller that imports one same-named definer must narrow to it"
    );

    // Carve-out 2: a qualified `.save(` anywhere in the body disables narrowing.
    assert_eq!(
        narrow(
            "def run(): obj.save(); save()",
            &mut ImportNarrowCache::default()
        ),
        None,
        "a qualified `.save(` must disable narrowing — prune keeps those edges"
    );

    // Carve-out 3: truncated body makes carve-out 2 a false negative, so the
    // caller is left alone (mirrors prune's `code_content NOT LIKE '%...'`).
    assert_eq!(
        narrow(
            "def run(): save() # body cut here ...",
            &mut ImportNarrowCache::default()
        ),
        None,
        "a truncated caller body must disable narrowing"
    );

    // A file that imports nothing by this name is exactly what prune leaves
    // alone: no narrowing, not an empty result.
    let other_file = mk_file("src/nonimporter.py");
    let lone = mk_fn(other_file, "run", "def run(): save()");
    assert_eq!(
        import_narrowed_targets(
            &db,
            "src/nonimporter.py",
            "save",
            &[lone],
